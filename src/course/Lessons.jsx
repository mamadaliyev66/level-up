...
export default function Lessons() {
  ...
  const [smallAlert,setSmallAlert] = useState()
  useEffect(() => {
      const auth = getAuth();
...
if(auth.currentUser){
  if (auth.currentUser.email) {
    ...
    const getLessonsForCourse = async (courseId) => {
      ...
      // console.log(r_users?.[0]?.ref);

      for (const doc of r_users_snapshot.docs) {
        const registeredUserDoc = doc.ref; // Get document reference directly
        // Update the document using registeredUserDoc
        await updateDoc(registeredUserDoc, {
          lesson_process: Number(currentLesson)  + 1, // Access data using doc.data()
        });
        setSmallAlert('')
        
        window.location.reload()
        // console.log("Lesson progress updated successfully!");
        break; // Exit the loop after updating the first matching user
      }
      // console.log(r_users);
      // const courseRef1 = doc(collection(firestore, 'regular_courses'), courseId);
      // const registeredUsersRef = collection(courseRef1, "registered_users");
      // const query = where('uid', '==', auth.currentUser.uid); // Find user by UID
      // const snapshot = await getDocs(query);
      // const registeredUsers = snapshot.docs.map((doc) => ({ ...doc.data() }));

      // if (r_users.length > 0) {
      //   const registeredUserDoc = r_users[0].ref; // Get user document reference

      //   await updateDoc(registeredUserDoc, {
      //     lesson_process: r_users[0].lesson_process + 1, // Increment lesson progress
      //   });}
...
// 
        ...
        // console.log(false);
        setSmallAlert('You must watch the video tutorial till the end ! ')
     ...
     role="status">
      ...
      <div>
        <div className='w-full grid grid-cols-2 sm:hidden bg-blue-500 text-right text-white'>
          ...
          <div>
            <button onClick={SideBarMenu} className='border-1 border-white p-2 m-2 rounded-lg mr-3'>
              ...
              <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 ">
                  ...
                  {upcomingLessons?.map((ulesson,i)=>{
                    return(
                      ...
                      {/* 'text-center hover:bg-gray-200 p-2   cursor-pointer transition-all duration-150' */}
                    <li  className=' relative p-2 transition-all cursor-pointer duration-150'>
                      <img className='w-32 mx-auto' src="https://cdn-icons-png.freepik.com/512/5726/5726470.png" alt="" />
                      <div className=' font-bold text-center mx-auto text-gray-900'>

                        TESTS
                      </div>

                      <div className='absolute top-[40%] left-[40%] rotate-45 bg-rose-700 p-2 text-white rounded-lg bg-opacity-70 hover:bg-opacity-100 hover:rotate-0 transition-all duration-1000 hover:scale-150' >
                        Soon
                      </div>
                    </li>
                  </ul>
